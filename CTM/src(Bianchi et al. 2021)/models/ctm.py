...
class CTM:
    ...
    def __init__(self, tp, dataset_folder, bow_size, contextual_size, inference_type="combined", n_components=10, model_type='prodLDA',
    ...
    def fit(self, train_dataset, validation_dataset=None, save_dir=None, verbose=False, patience=5, delta=0, viz_freq=50):
        """
        Train the CTM model.
        ...
        if self.validation_data is not None:
            self.early_stopping = EarlyStopping(patience=patience, verbose=True, path=save_dir, delta=delta)
        ...
        for epoch in range(self.num_epochs):
            ...
            # here save lda vis of current status
            if epoch % viz_freq == 0:
                # visualization of the trained model:
                lda_vis_data = self.get_ldavis_data_format(self.vocab, self.train_data, n_samples=5) 
                visualization_tm = pyLDAvis.prepare(**lda_vis_data)
                pyLDAvis.save_html(visualization_tm, f'{self.dataset_folder}training_progress/CTM_topic_overview_{self.dataset_folder[:-1]}_n{self.n_components}_afterEpoch{epoch+1}.html')
                

        pbar.close()

        #return all train losses
    ...
    def get_word_distribution_by_topic_id(self, topic): #TODO use this feature HERE
        """
