...
def main():
    ...
    parser.add_argument('--out_file', default='out.txt',
                        help='model prediction per doc on the test corpus') 
    parser.add_argument('--doc_topic_distribution', default='out.txt',
                        help='model predictions on the test corpus')
    ...
    trainer.mcp(top_pred_num=args.top_pred_num, match_threshold=20, epochs=args.mcp_epochs) #Change back to args.match_threshold which is 20
    # Self-training 
    trainer.self_train(epochs=args.self_train_epochs, loader_name=args.final_model) 
    # Write test set results
    if args.test_file is not None:
        trainer.write_results(loader_name=args.final_model, out_file=args.out_file) # model predictions on the test corpus
        trainer.write_test_topic_distribution(loader_name=args.final_model, out_file=args.doc_topic_distribution)
