...
class LOTClassTrainer(object):
    ...
    def __init__(self, args):
        ...
        self.pretrained_lm = 'bert-base-multilingual-cased'#'bert-base-multilingual-uncased' # originally 'bert-base-uncased'
        self.tokenizer = BertTokenizer.from_pretrained(self.pretrained_lm, do_lower_case=False) # we are using cased version
        ...
        self.temp_dir = str(f'tmp_{self.dist_port}')
        ...
        try:
            for batch in wrap_label_name_dataset_loader:
                with torch.no_grad():
                    ...
                    match_idx = label_pos >= 0 # What does that mean? #TODO
    ...
    def category_vocabulary(self, top_pred_num=50, category_vocab_size=100, loader_name="category_vocab.pt"):
        ...
        if os.path.exists(loader_file):
            ...
            with open('cat_vocabulary.txt', 'w', encoding='utf-8') as outfile:
                for i, category_vocab in self.category_vocab.items():
                    ...
                    cat_vocab = [self.inv_vocab[w] for w in category_vocab]
                    outfile.write(str(cat_vocab))
                    outfile.write('\n')
                ...
                with torch.no_grad():
                    ...
                    for i, category_vocab in self.category_vocab.items():
                        ...
                        for word_id in category_vocab:
                            match_idx = (sorted_res == word_id) | match_idx #HERE to cont.
    ...
    def prepare_mcp(self, top_pred_num=50, match_threshold=20, loader_name="mcp_train.pt"):
        ...
        if os.path.exists(loader_file):
            ...
            self.mcp_data = {"input_ids": all_input_ids, "attention_masks": all_input_mask, "labels": all_mask_label} #TODO
    ...
    def self_train_batches(self, rank, model, self_train_loader, optimizer, scheduler, test_dataset_loader):
        ...
        model.zero_grad()
        try:
            for j, batch in enumerate(wrap_train_dataset_loader): #here
                input_ids = batch[0].to(rank)
                input_mask = batch[1].to(rank)
            ...
            if rank == 0:
                ...
                print(f"Average training loss: {avg_train_loss.mean().item()}")
                if self.with_test_label:
                    out_file = os.path.join(self.dataset_dir, 'test_accuracy.txt')
    ...
    def inference(self, model, dataset_loader, rank, return_type):
        ...
        try:
            ...
            if return_type == "data":
                all_input_ids = torch.cat(all_input_ids, dim=0)
                all_input_mask = torch.cat(all_input_mask, dim=0)
                all_preds = torch.cat(all_preds, dim=0) #TODO HERE
    ...
    # use trained model to calculate accuracy of predictions on the test set
    def write_acc_results(self, loader_name, out_file):
        loader_file = os.path.join(self.dataset_dir, loader_name)
        assert os.path.exists(loader_file)
        print(f"\nLoading final model from {loader_file}")
        self.model.load_state_dict(torch.load(loader_file))
        ...
        test_set = TensorDataset(self.test_data["input_ids"], self.test_data["attention_masks"])
        test_dataset_loader = DataLoader(test_set, sampler=SequentialSampler(test_set), batch_size=self.eval_batch_size)
        accuracy = self.inference(self.model, test_dataset_loader, 0, return_type="acc")
        gather_acc = [torch.ones_like(accuracy) for _ in range(self.world_size)]
        dist.all_gather(gather_acc, accuracy)
        acc = torch.tensor(gather_acc).mean().item()
        out_file = os.path.join(self.dataset_dir, out_file)
        print(f"Writing test accuracy results to {out_file}")
        f_out = open(out_file, 'w')
    ...
    # use trained model to get all predictions on the test set
    def write_test_topic_distribution(self, loader_name, out_file):
        loader_file = os.path.join(self.dataset_dir, loader_name)
        assert os.path.exists(loader_file)
        print(f"\nLoading final model from {loader_file}")
        self.model.load_state_dict(torch.load(loader_file))
        ...
        test_set = TensorDataset(self.test_data["input_ids"], self.test_data["attention_masks"])
        test_dataset_loader = DataLoader(test_set, sampler=SequentialSampler(test_set), batch_size=self.eval_batch_size)
        all_input_ids, all_input_mask, all_preds = self.inference(self.model, test_dataset_loader, 0, return_type="data")
        out_file = os.path.join(self.dataset_dir, out_file)
        print(f"Writing distributions to {out_file}")
        all_preds = [t.cpu().numpy() for t in all_preds]
        np.savetxt(out_file, all_preds, delimiter=" ", newline='\n')
        '''f_out = open(out_file, 'w')
        for top in all_preds:
            f_out.write(str(top) + '\n')'''
